  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(WAITLIST_ENDPOINT, {
        method: "POST",
        headers: {
          // Keep the body as JSON, but use a simple content type so browsers
          // can submit to Google Apps Script without a failing CORS preflight.
          ...
          <div className="grid gap-5 xl:grid-cols-3">
            {copy.presets.items.map((preset) => (
              ...
              >
                ...
                <p classsName="mt-4 leading-7 text-slate-300">
      ...
      <section id="join-launch" className="section-divider scroll-mt-28">
        ...
        >
          <div className="rounded-[2.4rem] border border-cyan-200/14 bg-[var(--surface-strong)] p-7 shadow-[0_22px_70px_rgba(2,6,23,0.44)] md:p-9">
            ...
            <p
              aria-live="polite"
              className={`mt-4 min-h-6 text-sm ${status === "error" ? "text-rose-300" : "text-cyan-100/82"}`}
            >
              {feedback}
            </p>
          </div>
        ...
        >
          <div className="space-y-4">
            {copy.faq.items.map((item, index) => {
              ...
              return (
                <div key={item.question.es} className="rounded-[1.8rem] border border-white/10 bg-[var(--surface)] px-6 py-5">
                  <button
                    type="button"
                  ...
                  {expanded ? (<p className="mt-4 max-w-3xl leading-7 text-slate-300">{getText(item.answer, locale)}</p>) : null}
                </div>
              );
            })}
          </div>
      ...
      <div className="panel-sheen relative w-full max-w-[34rem] overflow-hidden rounded-[2.8rem] border border-cyan-200/12 bg-linear-to-b from-white/8 via-[#07111f]/92 to-[#050b16] p-6 shadow-[0_26px_90px_rgba(4,8,20,0.78)] sm:p-8">
        <div className="grid gap-7">
          ...
          <div className="relative h-[22rem] overflow-hidden rounded-[2rem] border border-white/8 bg-[radial-gradient(circle_at_center,rgba(25,139,255,0.18),transparent_45%),linear-gradient(180deg,#07101c_0%,#03070f_100%)]">
            ...
            <div className="animate-bars wh-5 rounded-full bg-linear-to-t from-blue-500 to-cyan-200 shadow-[0_0_24px_rgba(120,240,255,0.26)]" style={{
              height: 48,
              animationDelay: "0ms",
            }} />
            <div className="absolute bottom-8 left-0 right-0 flex items-end justify-center gap-3 px-8">
              {[48, 82, 66, 104, 78, 92, 58].map((height, index) => (
                <span key={height} className="animate-bars w-5 rounded-full bg-linear-to-t from-blue-500 to-cyan-200 shadow-[0_0_24px_rgba(120,240,255,0.26)]" style={{ height, animationDelay: `${index * 140}ms`, }} />
             ))}
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {copy.metrics.map((metric) => (
              <div key={metric.label.es} className="rounded-[1.4rem] border border-white/8 bg-white/5 px-4 py-4">
      ...
      <div className="rounded-[2.6rem] border border-white/10 bg-[var(--surface-strong)] p-8 shadow-[0_24px_80px_rgba(3,7,18,0.48)] md:p-10">
        ...
        <div className="mt-12 grid gap-5">
          {content.sections.map((section) => (
            <article key={section.title.es} className="rounded-[2rem] border border-white/8 bg-white/4 p-6">
        ...
        <div className="mt-10 flex flex-wrap gap-4">
          {content.actions.map((action) => (
            <Link key={action.href} href={action.href} className={`inline-flex min-h-13 items-center justify-center rounded-full px-6 text-sm font-semibold uppercase tracking-[0.24em] transition ${action.variant === "primary" ? "bg-linear-to-r from-cyan-300 to-blue-500 text-slate-950 shadow-[0_18px_48px_rgba(25,139,255,0.28)] hover:scale-[1.01]" : "border border-white/12 bg-white/6 text-slate-100 hover:border-cyan-200/34 hover:bg-white/10"}`}>
...
function BenefitIcon({ icon }: { icon: "dialogue" | "volume" | "cinema" | "platforms" }) {
  const commonClassName = "h-14 w-14 rounded-2xl border border-cyan-200/16 bg-cyan-300/10 p-3 text-cyan-100";

  switch (icon) {
    case "dialogue":
      return (<div className={commonClassName}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-full w-full"><path d="M5 7.5h14M5 12h9M5 16.5h6" strokeWidth="1.7" strokeLinecap="round" /><path d="M4 4h16v13H8l-4 3V4Z" strokeWidth="1.5" strokeLinejoin="round" /></svg></div>);
    case "volume":
      return (<div className={commonClassName}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-full w-full"><path d="M5 10h4l5-4v12l-5-4H5z" strokeWidth="1.5" strokeLinejoin="round" /><path d="M17 9c1.2 1 2 2.6 2 4s-.8 3-2 4" strokeWidth="1.7" strokeLineCap="round" /><path d="M19.5 6.5A8.4 8.4 0 0 1 22 13a8.4 8.4 0 0 1-2.5 6.5" strokeWidth="1.7" strokeLineCap="round" /></svg></div>);
    case "cinema":
      return (<div className={commonClassName}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-full w-full"><rect x="4" y="5" width="16" height="14" rx="2.5" strokeWidth="1.5" /><path d="M8 5v14M16 5v14M4 9h16M4 15h16" strokeWidth="1.5" /></svg></div>);
    case "platforms":
      return (<div className={commonClassName}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-full w-full"><path d="M4 7.5h16M7 4v3.5M17 4v3.5" strokeWidth="1.5" strokeLineCap="round" /><rect x="4" y="7" width="16" height="12" rx="2.5" strokeWidth="1.5" /><path d="M8 12h3v3H8zM13 12h3v3h-3z" strokeWidth="1.5" /></svg></div>);
  }
}
